...
| `external discovery evidence` | A second live `exa_answer` rerun on `2026-05-08`, after the Trust Chain disambiguation and founder-linkage copy was deployed, still produced `0/10` W&Patent mentions and `0/10` W&Patent citations. | The first copy-only improvement pass was not enough to change external answer behavior on its own. |
| `external discovery evidence` | The post-deploy Exa answer for `trust chain for websites` still resolved to TLS or certificate-chain framing. | Disambiguation on the current pages was necessary, but it has not yet been strong enough to win the concept externally without stronger topic ownership or broader authority signals. |
...
| --- | --- | --- | --- | --- | --- |
| `P1` | `Trust Chain` still defaults to security-language meanings outside W&Patent context. | `opencode_dev` and both `exa_answer` snapshots drifted toward certificate-chain framing on the plain prompt `trust chain for websites`, even after the live disambiguation update. | `trust-chain.htm`, `trust-chain-explainer.htm`, homepage Trust Chain block, plus future topic pages | Keep the new disambiguation language, but stop treating copy refinement alone as the main lever. Pair it with stronger supporting pages, internal links, and adjacent topic coverage so W&Patent earns more surrounding context. | Rerun `trust chain for websites` without URL constraints and check for fewer security-only answers plus more W&Patent mentions or citations. |
...
| `P1` | W&Patent is absent from high-value non-branded prompt clusters. | Both Exa snapshots returned `0/10` W&Patent mentions across patent strategy, defensibility, commercialization, advisory, authority, and AI-readability prompts. | New public topic pages plus stronger links from `index.html`, `about.htm`, and `trust-chain-explainer.htm` | Build dedicated pages for the highest-intent prompt clusters W&Patent wants to own, especially `startup patent strategy`, `patent commercialization for founders`, `patent advisory for startup founders`, `founder-led authority building`, and `AI-readable company website`. | Rerun the full prompt set after publishing the new pages and compare mention and citation rates by topic cluster. |
...
1. Keep the new Trust Chain disambiguation and founder-linkage changes live as the new baseline.
